...
class stockdata:
	def __init__(self):
		self.dbengine=create_engine('mysql+pymysql://baostock:123456@localhost/baostock?charset=utf8')
		self.fundnames={}
		self.threadPool=ThreadPoolExecutor(max_workers=5, thread_name_prefix="thread_")
	def kday_data(self,stockCode,startDate):

'''
lg = bstock.login()

engine = create_engine('mysql+pymysql://baostock:123456@localhost/baostock?charset=utf8')

rs = bstock.query_history_k_data_plus("sh.600000",
...
result.to_sql('kday', engine,index = False,if_exists = 'append')
